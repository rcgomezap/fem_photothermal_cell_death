...
# REGION PROPERTIES
regions = {
        'epidermis': [21, #GMSH physical tag
...
def is_laser_on(t: float, period: float) -> bool:
    """Check if laser should be on at time t given the period."""
    return (t % period) < (period / 2)

def get_heat_source(t: float) -> Function:
    """Get heat source function at time t."""
    # Check if within irradiation time
    if t > parameters['laserProperties']['irradiationTime']:
        return Function(V)
    
    # Apply laser switching
    laser_period = parameters['laserProperties']['period']
    if is_laser_on(t, laser_period):
        return Qs
    else:
        return Function(V)
...
solT = solve_DPL(V = V,msh = mesh[0],T = T,v = v,ds = ds,
                ...
                Qmet = Qmet,blood = blood,Qs_func = get_heat_source,h = h,
